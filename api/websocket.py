...
async def websocket_agent_endpoint(websocket: WebSocket):
    ...
    max_silent_frames = int(
        (settings.SILENCE_DURATION_MS / 1000.0) / (VAD_FRAME_SIZE_BYTES / 2 / settings.SAMPLE_RATE)
    )
    ...
    try:
        while True:
            ...
            while len(network_buffer) >= VAD_FRAME_SIZE_BYTES:
                ...
                # ==========================================
                if fsm.current_state in (AgentState.SPEAKING, AgentState.PROCESSING) and speech_prob > settings.VAD_THRESHOLD:
                    logger.warning("Barge-in detected! Halting generation and flushing ESP32 buffers.")
                ...
                if speech_prob > settings.VAD_THRESHOLD:
                    ...
                    if silent_frames_count >= max_silent_frames:
                        ...
                        # Fire-and-forget task structure to allow continued WebSocket monitoring
                        async def process_and_speak():
                            try:
                                # 1. STT (Whisper.cpp / Parakeet) [24, 61]
                                user_prompt = await audio_pipeline.transcribe(final_audio_bytes)
